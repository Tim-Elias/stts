...
class PromptManager:
    ...
    def get_prompts_by_user(self, user):
        session = self.Session()
        try:
            logger.info(f"Получение промптов для пользователя: {user}", extra={'user_id': 'PromptManager'})
    ...
    def get_automatic_prompt(self, user):
        session = self.Session()
        try:
            logger.info(f"Поиск автоматического промпта для пользователя: {user}", extra={'user_id': 'PromptManager'})
            ...
            if prompt:
                logger.info(f"Автоматический промпт '{prompt.prompt_name}' найден для пользователя: {user}", extra={'user_id': 'PromptManager'})
                return {
                    "prompt_name": prompt.prompt_name,
                    "text": prompt.text,
                    "prompt_id": prompt.prompt_id,
                ...
                logger.info(f"Автоматический промпт не найден для пользователя: {user}", extra={'user_id': 'PromptManager'})
                return None
        except Exception as e:
    ...
    def reset_automatic_flag(self, user):
        logger.info(f"Сброс флага 'use_automatic' для всех промптов пользователя {user}.")
        session = self.Session()
        try:
            ...
            session.commit() 
            logger.info(f"Флаг 'use_automatic' сброшен для {len(prompts)} промптов пользователя {user}.")
        except Exception as e:
            logger.error(f"Ошибка при сбросе флага 'use_automatic' для пользователя {user}: {e}")
            session.rollback()
            raise e
        finally:
            session.close()
            logger.info(f"Сессия для сброса флагов 'use_automatic' для пользователя {user} закрыта.")
