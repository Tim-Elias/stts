...
def get_prompts():
    ...
    logger.info(f"Загрузка страницы управления промптами для пользователя: {user}")
    ...
    for s in prompts:
        ...
        logger.info(f"Загрузка страницы управления промптами для пользователя: {prompt_info}")
...
def add_prompt():
    ...
    try:
        logger.info(f"Добавление нового промпта для пользователя {user}: Название: {prompt_name}")
...
def delete_prompt(prompt_id):
    ...
    user = get_jwt_identity()
    try:
        logger.info(f"Удаление промпта {prompt_id} для пользователя {user}")
        prompt_manager.delete_prompt(prompt_id)
        logger.info(f"Промпт {prompt_id} удален для пользователя {user}")
...
def edit_prompt(prompt_id):
    ...
    try:
        logger.info(f"Изменение промпта {prompt_id} для пользователя {user}")
        success = prompt_manager.edit_prompt(prompt_id, new_text, new_prompt_name)  # Передаем новое имя

        if success:
            logger.info(f"Промпт {prompt_id} изменен для пользователя {user}")
            return jsonify(success=True, message='Prompt updated successfully'), 200
        else:
            logger.error(f"Промпт {prompt_id} не найден или произошла ошибка")
